...
nova remove-secgroup b default
nova add-secgroup b ssh

nova interface-attach --port-id $(neutron port-show -F id -f value ffhs_net_a_port1) c
nova interface-attach --port-id $(neutron port-show -F id -f value ffhs_net_a_port2) a
nova interface-attach --port-id $(neutron port-show -F id -f value ffhs_net_b_port1) c
nova interface-attach --port-id $(neutron port-show -F id -f value ffhs_net_b_port2) b

openstack server add floating ip c $(get_floating_ip)
openstack server add floating ip a $(get_floating_ip)
openstack server add floating ip b $(get_floating_ip)
